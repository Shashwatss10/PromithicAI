  ...
  var HistoryManager = {
    ...
    add: function (item) {
      ...
      if (user && window.SupabaseDB) {
        window.SupabaseDB.saveBuild(user.uid, newItem).catch(function (err) {
     ...
     */
    remove: function (id) {
      var items = safeGet().filter(function (i) {
        return i.id !== id;
      });
      safeSet(items);
      window.dispatchEvent(new CustomEvent("historyUpdated"));
      ...
      var user = window.currentUser;
      if (user && window.SupabaseDB) {
        window.SupabaseDB.deleteBuild(user.uid, id).catch(function (err) {
    ...
    clear: function () {
      ...
      var user = window.currentUser;
      if (user && window.SupabaseDB) {
        window.SupabaseDB.clearBuilds(user.uid).catch(function (err) {
        ...
        .map(function (item) {
          ...
          return [
            ...
            '    <div class="history-time">' +
              ...
              "</div>",
            "  </div>",
            '  <button class="history-delete" data-delete-id="' +
              item.id +
              '" title="Delete">',
            '    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">',
